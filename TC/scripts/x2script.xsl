...
<xsl:template match="/">
	<html>
	<head>
	<title>X3TC Script: <xsl:value-of select="script/name"/></title>
	</head>
...
<xsl:template match="sourcetext">
  ...
  <xsl:template match="line">
    <b style="color:blue"><xsl:value-of select="@linenr"/>&#160;<xsl:choose><xsl:when test="@interruptable"><b style="color:cyan"><xsl:value-of select="@interruptable"/></b></xsl:when><xsl:otherwise>&#160;</xsl:otherwise></xsl:choose></b>&#160;<b style="color:#777777"><xsl:value-of select="translate(@indent,'&#160;','|&#255;')"/></b><xsl:apply-templates/><br/>
  </xsl:template>
